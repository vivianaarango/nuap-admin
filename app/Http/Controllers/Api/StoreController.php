...
class StoreController
{
     ...
     * @type string
     */
    protected const STORE_NOT_FOUND = 'STORE_NOT_FOUND';
    ...
    public function __invoke(Request $request, int $address): JsonResponse
    {
        try {
            ...
            if ($user->role === User::COMMERCE_ROLE) {
                $stores = $this->dbProductRepository->getEnabledStoresByDistributor();
            } else {
                $stores = $this->dbProductRepository->getEnabledStoresByCommerce();
            }
            ...
            if (!count($storeList)) {
                $error = new ErrorObject();
                $error->setCode(self::STORE_NOT_FOUND)
                    ->setTitle(self::ERROR_TITLE)
                    ->setDetail('No se encontraron tiendas.')
                    ->setStatus((string) Response::HTTP_BAD_REQUEST);
                $this->jsonErrorFormat->add($error);
            ...
            $this->httpObject->setCollection($stores->toArray());
